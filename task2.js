(function() {
    // Вспомогательная функция для сверхточной генерации графиков по координатам из LaTeX
    const makeSvg = (minX, minY, maxX, maxY, vecs) => {
        let sc = 25; // Масштаб (пикселей на одну клетку)
        let w = (maxX - minX) * sc, h = (maxY - minY) * sc;
        let tx = x => (x - minX) * sc;
        let ty = y => h - (y - minY) * sc;
        
        let s = '<svg viewBox="0 0 ' + w + ' ' + h + '" width="' + w + '" height="' + h + '" style="max-width:100%; height:auto;">';
        
        // Сетка
        for(let x = Math.ceil(minX); x <= Math.floor(maxX); x++) {
            s += '<line x1="'+tx(x)+'" y1="0" x2="'+tx(x)+'" y2="'+h+'" stroke="#e0e0e0" stroke-width="1"/>';
        }
        for(let y = Math.ceil(minY); y <= Math.floor(maxY); y++) {
            s += '<line x1="0" y1="'+ty(y)+'" x2="'+w+'" y2="'+ty(y)+'" stroke="#e0e0e0" stroke-width="1"/>';
        }
        
        // Оси
        if (minY <= 0 && maxY >= 0) {
            s += '<line x1="0" y1="'+ty(0)+'" x2="'+w+'" y2="'+ty(0)+'" stroke="#000" stroke-width="1.5" marker-end="true"/>';
            s += '<text x="'+(w-15)+'" y="'+(ty(0)-8)+'" font-family="Times New Roman" font-size="16" font-style="italic">x</text>';
        }
        if (minX <= 0 && maxX >= 0) {
            s += '<line x1="'+tx(0)+'" y1="'+h+'" x2="'+tx(0)+'" y2="0" stroke="#000" stroke-width="1.5" marker-end="true"/>';
        ...
        // Отрисовка векторов
        for(let v of vecs) {
            s += '<line x1="'+tx(v.x1)+'" y1="'+ty(v.y1)+'" x2="'+tx(v.x2)+'" y2="'+ty(v.y2)+'" stroke="#000" stroke-width="2.5" marker-end="true"/>';
            
            // Расчет позиции подписи с учетом смещения
            let lx = (tx(v.x1)+tx(v.x2))/2 + v.dx;
            let ly = (ty(v.y1)+ty(v.y2))/2 + v.dy;
            
            // Название вектора
            s += '<text x="'+lx+'" y="'+ly+'" font-family="Times New Roman" font-size="18" font-style="italic">'+v.name+'</text>';
            // Рисуем значок вектора над буквой жесткими путями (чтобы autoFixSVGArrows его не трогал)
        ...
        prototypes: [
            {
                ...
                theory_sol: "Формула длины вектора: $|\\vec{a}| = \\sqrt{x^2+y^2}$.<br>$|\\vec{a}| = \\sqrt{8^2 + (-6)^2} = \\sqrt{64+36} = \\sqrt{100} = 10$.",
                tasks: [
            ...
            },
            {
                ...
                theory_sol: "1) Найдём координаты вектора $2\\vec{b}$: $(2 \\cdot 2; 2 \\cdot 3) = (4; 6)$.<br>2) Найдём координаты разности векторов $\\vec{a} - 2\\vec{b}$: $(4 - 4; 1 - 6) = (0; -5)$.<br>3) Вычислим длину полученного вектора: $\\sqrt{0^2 + (-5)^2} = \\sqrt{25} = 5$.",
                tasks: [
            ...
            {
                desc: "Прототип 3",
                svg_code: makeSvg(-1.5, -1.5, 6.5, 7.5, [ {x1:1, y1:1, x2:4, y2:7, name:'a', dx:-20, dy:-5}, {x1:1, y1:1, x2:6, y2:1, name:'b', dx:0, dy:20} ]),
                theory_task: "На координатной плоскости изображены векторы $\\vec{a}$ и $\\vec{b}$. Найдите длину вектора $\\vec{a}+\\vec{b}$.",
                theory_sol: "1) Определим координаты по чертежу (конец минус начало):<br>$\\vec{a} = (4-1; 7-1) = (3; 6)$.<br>$\\vec{b} = (6-1; 1-1) = (5; 0)$.<br>2) Сложим векторы: $\\vec{a}+\\vec{b} = (3+5; 6+0) = (8; 6)$.<br>3) Длина вектора: $\\sqrt{8^2 + 6^2} = \\sqrt{64+36} = \\sqrt{100} = 10$.",
                tasks: [
                    { 
                        ...
                        svg_code: makeSvg(-1.5, -1.5, 6.5, 5.5, [ {x1:1, y1:1, x2:3, y2:4, name:'a', dx:-25, dy:-10}, {x1:4, y1:4, x2:6, y2:3, name:'b', dx:15, dy:5} ]),
                        answer: "8" 
                    },
                    { 
                        ...
                        svg_code: makeSvg(-1.5, -1.5, 6.5, 5.5, [ {x1:1, y1:2, x2:4, y2:4, name:'a', dx:-20, dy:-10}, {x1:2, y1:1, x2:3, y2:3, name:'b', dx:10, dy:5} ]),
                        answer: "2" 
                    },
                    { 
                        ...
                        svg_code: makeSvg(-1.5, -1.5, 5.5, 5.5, [ {x1:2, y1:1, x2:1, y2:4, name:'a', dx:-20, dy:0}, {x1:3, y1:1, x2:5, y2:4, name:'b', dx:15, dy:0} ]),
                        answer: "5" 
                    },
                    { 
                        text: "На координатной плоскости изображены векторы $\\vec{a}$ и $\\vec{b}$, координатами которых являются целые числа. Найдите длину вектора $-\\vec{a}+3\\vec{b}$.", 
                        svg_code: makeSvg(-1.5, -1.5, 7.5, 8.5, [ {x1:1, y1:1, x2:1, y2:7, name:'a', dx:-20, dy:0}, {x1:3, y1:2, x2:6, y2:4, name:'b', dx:15, dy:20} ]),
                        answer: "9" 
                    },
                    { 
                        ...
                        svg_code: makeSvg(-1.5, -1.5, 6.5, 7.5, [ {x1:1, y1:2, x2:4, y2:7, name:'a', dx:-20, dy:0}, {x1:2, y1:0, x2:6, y2:1, name:'b', dx:0, dy:20} ]),
                        answer: "17" 
                    }
                ]
            },
            {
                ...
                theory_sol: "Скалярное произведение через угол: $\\vec{a}\\cdot\\vec{b} = |\\vec{a}| \\cdot |\\vec{b}| \\cdot \\cos \\angle(\\vec{a}, \\vec{b})$.<br>$\\vec{a}\\cdot\\vec{b} = 3 \\cdot 4 \\cdot \\cos 60^{\\circ} = 12 \\cdot 0{,}5 = 6$.",
                tasks: [
            ...
            },
            {
                ...
                theory_sol: "Скалярное произведение в координатах: $\\vec{a}\\cdot\\vec{b} = x_1 x_2 + y_1 y_2$.<br>$\\vec{a}\\cdot\\vec{b} = 2 \\cdot 3 + (-5) \\cdot 1 = 6 - 5 = 1$.",
                tasks: [
            ...
            {
                desc: "Прототип 6",
                svg_code: makeSvg(-1.5, -1.5, 6.5, 5.5, [ {x1:1, y1:1, x2:3, y2:4, name:'a', dx:-20, dy:0}, {x1:1, y1:2, x2:5, y2:1, name:'b', dx:0, dy:20} ]),
                theory_task: "На координатной плоскости изображены векторы $\\vec{a}$ и $\\vec{b}$. Найдите скалярное произведение $\\vec{a}\\cdot\\vec{b}$.",
                theory_sol: "1) Определим координаты векторов:<br>$\\vec{a} = (3-1; 4-1) = (2; 3)$.<br>$\\vec{b} = (5-1; 1-2) = (4; -1)$.<br>2) Перемножим векторы: $\\vec{a}\\cdot\\vec{b} = 2 \\cdot 4 + 3 \\cdot (-1) = 8 - 3 = 5$.",
                tasks: [
                    { 
                        ...
                        svg_code: makeSvg(-1.5, -1.5, 6.5, 5.5, [ {x1:1, y1:1, x2:4, y2:5, name:'a', dx:-25, dy:-10}, {x1:2, y1:1, x2:6, y2:2, name:'b', dx:5, dy:20} ]),
                        answer: "16" 
                    },
                    { 
                        text: "На координатной плоскости изображены векторы $\\vec{a}$ и $\\vec{b}$, координатами которых являются целые числа. Найдите скалярное произведение $\\vec{a}\\cdot\\vec{b}$.", 
                        svg_code: makeSvg(-1.5, -1.5, 6.5, 6.5, [ {x1:1, y1:4, x2:5, y2:6, name:'a', dx:-25, dy:-10}, {x1:1, y1:2, x2:4, y2:1, name:'b', dx:-20, dy:20} ]),
                        answer: "10" 
                    },
                    { 
                        ...
                        svg_code: makeSvg(-1.5, -1.5, 6.5, 5.5, [ {x1:2, y1:1, x2:1, y2:4, name:'a', dx:-20, dy:0}, {x1:3, y1:1, x2:6, y2:4, name:'b', dx:15, dy:0} ]),
                        answer: "12" 
                    },
                    { 
                        text: "На координатной плоскости изображены векторы $\\vec{a}$ и $\\vec{b}$, координатами которых являются целые числа. Найдите скалярное произведение $\\vec{a}\\cdot(-\\vec{b})$.", 
                        svg_code: makeSvg(-1.5, -1.5, 6.5, 5.5, [ {x1:1, y1:2, x2:4, y2:5, name:'a', dx:-20, dy:0}, {x1:5, y1:2, x2:3, y2:4, name:'b', dx:15, dy:0} ]),
                        answer: "0" 
                    },
                    { 
                        ...
                        svg_code: makeSvg(-1.5, -1.5, 6.5, 5.5, [ {x1:2, y1:1, x2:4, y2:4, name:'a', dx:-25, dy:-10}, {x1:4, y1:1, x2:3, y2:3, name:'b', dx:15, dy:0} ]),
                        answer: "8" 
                    }
                ]
            }
